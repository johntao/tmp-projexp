...
export class TtDial extends HTMLElement {
  ...
  /** Start external drag (no pointer capture). */
  beginDrag() {
    this._dragging = true;
    this._cancelled = false;
  }

  /** Feed an external pointermove into the dial. Uses e.clientX/clientY. */
  trackMove(e) {
    this._onPointerMove(e);
  }

  /** End external drag, fires dial-commit or dial-cancel. */
  endDrag() {
  ...
  _stepToAngle(step) { return (step * 30 - 90) * Math.PI / 180; }
  _angleToStep(angle) {
  ...
  _onPointerDown(e) {
    ...
    const step = this._angleToStep(this._getPointerAngle(e));
    this._lastStep = this._currentStep;
  ...
  _render() {
    const cx = 110, cy = 110, r = 90, cancelR = 25, stepR = r - 12;
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const mx = cx + stepR * Math.cos(a), my = cy + stepR * Math.sin(a);
    ...
    if (!this._cancelled) {
      ...
      html += `<line class="hand" x1="${cx}" y1="${cy}" x2="${cx + (stepR - 8) * Math.cos(a)}" y2="${cy + (stepR - 8) * Math.sin(a)}" />`;
    }
    ...
    if (this._cancelled) html += `<text class="label-text" x="${cx}" y="${cy + 34}" style="fill:#d63851">CANCEL</text>`;
